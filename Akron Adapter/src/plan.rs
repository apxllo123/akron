...
pub fn build_plan(profile: &GameProfile) -> AdaptationPlan {
    ...
    add_step(&mut steps, &mut modules, "analyze", "Analyze game", "Review the detected executable, graphics, API, runtime, and protection requirements before changing files.", "core.analyzer", StepStatus::Ready);
    ...
    modules.sort();
    modules.dedup();
    AdaptationPlan { steps, required_modules: modules }
}
