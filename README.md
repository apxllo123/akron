...
Its job is to build a machine-readable understanding of a game directory without modifying the source files. The current implementation can recursively inventory files, record metadata and SHA-256 hashes, identify executable candidates, detect PE binaries, and identify PE target architectures.
...
The Adapter is the adaptation and conversion layer that will consume Analyzer results.
...
The long-term Adapter is intended to coordinate compatibility decisions, packaging, runtime preparation, file transformation, target-platform application generation, and post-conversion validation.

### Akron Desktop

Akron's user interface is built with Electron and TypeScript.

The desktop layer is intentionally separated from the Rust engines. The Electron renderer runs with Node integration disabled and communicates through a controlled preload bridge, while the main process handles native dialogs and launches the Analyzer process.
...
- Complete runtime detection
- Graphics/API dependency analysis
- Automatic conversion planning
- Universal game conversion
- Converted-game validation and launch testing
...
`.github/workflows/build.yml` builds the Rust Analyzer, bundles it into the Electron application, and produces:

- a macOS ARM64 `Akron.app` ZIP artifact
- a Windows x64 installer artifact
