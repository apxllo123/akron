...
# Keep one canonical icon pipeline for Electron and the native macOS host.
node "$ROOT/scripts/prepare-icons.mjs"
ICON_ICNS="$ROOT/build/icon.icns"
if [[ ! -s "$ICON_ICNS" ]]; then
  echo "Missing generated Akron icon: $ICON_ICNS" >&2
  exit 1
fi
cp "$ICON_ICNS" "$RESOURCES/Akron.icns"
...
Icon: Akron.icns generated by prepare-icons.mjs
...
/usr/bin/file "$RESOURCES/Akron.icns"
